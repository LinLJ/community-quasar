...
<script>
import { postPublish,getPost, isCurrentPostAuthor } from '@/api/forum'
...
export default {
  ...
  methods: {
    //添加标签
    getPost() {
      isCurrentPostAuthor(this.$route.params.id).then(response => {
      this.isAuthor = response.data
      if(this.isAuthor){

        debugger
        getPost(this.$route.params.id).then(response => {
          this.post = response.data.contentInfo
          this.$refs.editor.setValue(this.post.content)
          //this.isAuthor=false
        }).catch(() => {
          console.info("编辑帖子导致错误")
        })
      }else {
